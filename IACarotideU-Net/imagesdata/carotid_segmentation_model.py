...
def load_data():
    ...
    if Y_masks:
        ...
        global CLASS_WEIGHT
        CLASS_WEIGHT = min(max(ratio, 10), 100)  # Limiter le poids entre 10 et 100
...
def visualize_results(image, true_mask=None, pred_mask=None, title="", output_path=None):
    if true_mask is not None:
        # Mode avec masque réel (validation)
        ...
        # Mode prédiction seulement (test)
...
# ====================== NOUVELLES FONCTIONS POUR LE TEST ======================
...
def predict_on_image(model, image_path, output_dir):
    ...
    visualize_results(image, None, binary_pred, f"Seuil: {THRESHOLD}", visualization_path)
...
def test_mode(model_path=MODEL_PATH, test_dir=TEST_DIR):
    """
    ...
    predict_on_directory(model, test_dir, test_output_dir)

# Fonction principale
def main():
    # Analyser les arguments de ligne de commande
    parser = argparse.ArgumentParser(description='Entraîner ou tester un modèle de détection de carotides')
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'test'], help='Mode: train ou test')
    parser.add_argument('--model', type=str, default=MODEL_PATH, help='Chemin vers le modèle pré-entraîné (pour le mode test)')
    parser.add_argument('--test_dir', type=str, default=TEST_DIR, help='Répertoire contenant les images de test (pour le mode test)')
    
    args = parser.parse_args()
    
    if args.mode == 'test':
        # Mode test
        test_mode(args.model, args.test_dir)
    else:
        # Mode entraînement
        ...
        print("Génération des prédictions de validation...")
